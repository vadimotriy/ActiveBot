...
class Data:
    ...
    def add_user(self, user_id: int, date: str):
        ...
        if (user_id,) not in all_users:
            self.cursor.execute(
                "INSERT INTO Settings (USER_ID, SWIMMING, BICYCLE, DATE, TASK_AMOUNT) VALUES (?, ?, ?, ?, ?)",
                (user_id, 0, 0, date, 0),
            )
    ...
    def change_settings(self, user_id: int, type_task: str, value: int):
        self.cursor.execute(f"UPDATE Settings SET {type_task} = ? WHERE USER_ID = ?", (value, user_id))
        self.connection.commit()
    ...
    def get_settings(self, user_id: int) -> tuple:
        result = self.cursor.execute(
            "SELECT SWIMMING, BICYCLE, DATE, TASK_AMOUNT FROM Settings WHERE USER_ID = ?", (user_id,)
        ).fetchone()
        return result
    ...
    def add_tasks(self, user_id: int, date: str, tasks_text: list[str]):
        self.cursor.execute(
            "INSERT INTO Tasks (USER_ID, TASK1, TEXT1, TASK2, TEXT2, TASK3, TEXT3, DATA) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
    ...
    def solve_task(self, user_id: int, date: str, task: int):
        self.cursor.execute(f"""UPDATE Tasks SET Task{task} = ? WHERE USER_ID = ? AND Data = ?""", (1, user_id, date))

        settings = self.get_settings(user_id)
        self.cursor.execute(f"""UPDATE Settings SET TASK_AMOUNT = ? WHERE USER_ID = ?""", (settings[3] + 1, user_id))
        self.connection.commit()
    ...
    def set_dairy(self, user_id: int, date: str, emotion: str, value: int) -> tuple:
        if not self.get_dairy(user_id):
            self.cursor.execute(
                "INSERT INTO Dairy (USER_ID, VERY_HAPPY, HAPPY, NORMAL, SAD, VERY_SAD, DATA) VALUES (?, ?, ?, ?, ?, ?, ?)",
                (user_id, 0, 0, 0, 0, 0, date),
            )
            self.connection.commit()

        self.cursor.execute(f"UPDATE Dairy SET {emotion.upper()} = ? WHERE USER_ID = ?", (value, user_id))
        self.connection.commit()
